...
**`market_data/individual_price.rs`** — Private implementation for display-time Individual price values for portfolio rows. Stocks use same-day Yahoo observations when available, ETFs use same-day observations supplied through their existing Morningstar price path, mutual funds retain Historical market data semantics, and Historical or snapshot fallback preserves row rendering when current display data is unavailable. Only observations dated today are treated as Live quotes.
