...
Rust CLI portfolio tracker with NAV unitization, multi-currency support, and ASCII charts. ~1,500 lines, edition 2021.
...
cargo run -- get --period ytd            # YTD chart (also: 3y, 5y, all)
...
- **Base currency**: EUR, hardcoded as `BASE_CURRENCY` in `src/services/exchange_rates.rs`. All portfolio values are converted to EUR for aggregation.
...
src/
  ...
  cli.rs               — Clap CLI definitions (get, buy, sell)
  display.rs           — Terminal tables (tabled) and NAV chart (textplots)
  lib.rs               — Public module exports
  models/
    asset.rs           — AssetType enum, Asset, AssetInfo, AssetPosition
    portfolio.rs       — PortfolioSnapshot, PortfolioSummary, PortfolioResult, PortfolioRow
    transaction.rs     — BuyOrder, SellOrder, Transaction, cents helpers
  services/
    nav.rs             — NAV unitization engine (rebuild_portfolio_history)
    portfolio.rs       — Portfolio summary, return calculations
    transactions.rs    — Buy/sell recording + snapshot invalidation
    ...
    metrics.rs         — Beta, Sharpe ratio (ACWI benchmark)
  db/
    mod.rs             — SQLite connection + auto-migration
    entities/          — SeaORM generated models (7 tables)
    repos/             — Repository layer (one per entity)
migration/src/         — SeaORM schema migrations
tests/                 — Integration tests + common test utilities
scripts/               — Python helpers for fund/ETF prices (mstarpy)
```
